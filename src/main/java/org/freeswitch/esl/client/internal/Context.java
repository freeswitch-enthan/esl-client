...
public class Context implements IModEslApi {
    ...
    @Override
    public List<CommandResponse> cancelEventSubscriptions(String... eventNames) {
        try {
            return Arrays.asList(eventNames).parallelStream().map(eventName -> {
                String command = "nixevent " + eventName;
                EslMessage response = getUnchecked(handler.sendApiSingleLineCommand(channel, command));
